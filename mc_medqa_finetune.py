...
#Define data path, model name(or path for BP finetuned models), and output dir
...
out_dir="biobert_original_direct"
...
def ray_hp_space(trial):
    ...
    return {
        ...
        "learning_rate": tune.choice([1e-6, 1e-5, 1e-4]),
...
def model_init():
    ...
    ### model checkpoint
    model = model_class.from_pretrained(model_path)
    ### RUN THE FOLLOWING LINE IF DO SEQUENTIAL FINE-TUNE
    #model.classifier = torch.nn.Linear(768, 1) #FOR BERT
    #model.classifier = torch.nn.Linear(2560, 1) #FOR BIOMEDLM
    return model
...
training_args = TrainingArguments(
    ...
    data_seed=42,
    weight_decay=0.01,
    #fp16=True, ### USE fp16 IF RUN ON SINGLE GPU
)
...
best_run = trainer.hyperparameter_search(direction="maximize", backend ='ray', compute_objective=my_objective, n_trials=12, hp_space = ray_hp_space,resources_per_trial={'cpu':12, 'gpu': 1},)
## https://discuss.huggingface.co/t/understanding-how-ray-objective-works-with-the-trainer/16313
#trainer.train()
print(best_run)
...
### checkpoint is from ray_result, objective_2023...., objective_(run_id)...(corresponding to the best run id),
## - check result.json find which epoch is the best result,
## - Save the checkpoint that yields best eval result for testing
"""
